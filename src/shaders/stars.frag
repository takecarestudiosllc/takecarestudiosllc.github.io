...
// spikes — before fading out entirely. Mostly-white with a whisper of the
// shared chroma() palette (prepended by src/shaders/index.ts) so the stars
// shimmer on the night sky without reading as colored.
...
void main() {
  ...
  if (a < 0.003) discard;
  gl_FragColor = vec4(mix(vec3(1.0), chroma(vHue), 0.18), a);
}
